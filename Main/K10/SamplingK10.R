####################
#### TITLE:     Sample subjects in K = 10 studies.
#### Contents:
####
...
# We have to sample 200 subjects each time.
...
# This should be loaded in the main_CBMA_K10.sh file.
...
wd <- 'PaperStudyCharCBMA/Main/K10'
setwd(wd)
...
# Number of runs/iterations that can be deployed
...
# For loop over runs/iterations and number of studies
for(r in 1:NRUNS){
	for(i in 1:NS){
...
save(StudySamSubj, file = paste(wd,'/StudySamSubjK10.RData',sep=''))

# Subjects in the Ref
save(RefSamSubj, file = paste(wd,'/RefSamSubjK10.RData',sep=''))
