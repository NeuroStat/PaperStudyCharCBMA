...
MACHINE <- as.character(args)[9]

if(MACHINE=='HPC'){
	source('/user/scratch/gent/gvo000/gvo00022/vsc40728/35Studies/functions.R')
}
if(MACHINE=='MAC'){
	...
	print("Need packages: oro.nifti, devtools and R file functions.R")
...
# # Significance level
# signLevel <- as.numeric(as.character(args)[9])
...
## Global Variables
	...
	templateMM <- 3
	# Own defined voxel size
	voxelSize <- 1
	# FWHM defined by Radua
	FWHM <- 20
...
##
###############
### Threshold at significance level
###############
##
...
# Threshold
ThrPVal <- array(0,dim=dim(PVal))
ThrPVal[which(PVal <= signLevel)] <- 1
save(ThrPVal,file=paste(outDir,"ThrPVal",sep=""))
print(paste("Thresholded ", metamethod, " Effects Meta-Analysis",sep=""))
}
