...
#### Contents:
####
#### Source Files: //Meta\ Analyis/R\ Code/Studie_CBMA/PaperStudyCharCBMA.git/Main/K20
#### First Modified: 05/09/2016 (K = 20 version, based on K = 10 version of 13/11/2014)
#################
...
SCRPT=PaperStudyCharCBMA/Main/K20
...
# We have run this on high performance cluster which takes 1-3 as argument.
# Hence this is run with value 1,2,3
RUN=1
...
cd "${SCRPT}"
Rscript "${SCRPT}"/LoadSubjK20.R "$Studies" "$RUN" "$SCRPT" &> ROutput_SamplingCrossVal.txt
